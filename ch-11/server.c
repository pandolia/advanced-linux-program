...
static char* bad_requese_response =
...
/* HTTP response, header, and body template, indicating that the requested
...
static void handle_connection(int connection_fd)
{
    ...
    if (bytes_read > 0) {
        ...
        /* Some data was read successfully. NUL-terminate the buffer
         * so we can use string operations on it. */
        buffer[bytes_read] = '\0';
         ...
         * which is delimited by a blank line. HTTP specifies CR/LF
        ...
        if (strcmp(protocol, "HTTP/1.0") &&
            ...
            write(connection_fd, bad_requese_response,
                  sizeof(bad_requese_response));
...
void server_run(struct in_addr local_address, uint16_t port)
{
    ...
    /* Install a handler for SIGCHILD that cleans up child processes
     * that have terminate. */
    ...
    /* Construct a socket address structure for the local address
     * on which we want to listen for connections. */
    ...
    if (verbose) {
        ...
        rval = getsockname(server_socket,
                           &socket_address, &address_length);
        assert(rval == 0);
    ...
    while (1) {
        ...
        /* Accept a connection. This call blocks until a connection
         * is ready. */
        ...
        if (child_pid == 0) {
            ...
            /* This is the parent process. The child process handles
             * the connection, so we don't need our copy of the connected
             * socket descriptor. Close it. The continue with the loop

...
/* Set *UID and *GID to the owning user ID and group ID,
 * respectinvely, of process PID. Return on success, nonzero
 * on failure. */
...
/* Return the name of user UID. The return value is a buffer
 * that the caller must deallocate with free. UID must be a valid
 * user ID. */
...
/* Return the name of the program running in process PID,
 * or NULL on error. The return value is a newly allocated
 * buffer which the caller must deallocate with free. */
static char* get_program_name(pid_t pid)
{
    ...
    /* Generate the name of the "stat" file in the process's
     * /proc directory, and open it. */
    snprintf(file_name, sizeof(file_name), "/proc/%d/stat",
             (int)pid);
    fd = open(file_name, O_RDONLY);
    if (fd == -1) {
        /* Couldn't open the stat file for this process. Perhaps
    ...
    /* The program name is the second element of the file contents
     * and is surrounded by parentheses. Find the positions of the
     * parentheses in the file contents. */
...
/* Return the resident set size (RSS), in kilobytes, of
 * process PID. Return -1 on failure. */
static int get_rss(pid_t pid)
{
    ...
    /* Generate the name of the proces's "statm" entry in its
     * /proc directory. */
    ...
    if (rval != 1) {
        /* The contents of statm are formatted in a way we
         * don't understand. */
        return -1;
    }

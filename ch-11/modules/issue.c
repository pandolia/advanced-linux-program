/* p11.7-issue.c
 * Server Module to Display GNU/Linux Distribution
 * Information.
 */
#include <fcntl.h>
#include <string.h>
...
/* HTML source for the page indicating there was a problem
 * opening /proc/issue. */
...
void module_generate(int fd)
{
    ...
    if (rval == -1) {
        /* Either we couldn't open the file or we couldn't
         * read from it. */
        ...
        /* Copy from /proc/issur to the client socket. */
        rval = sendfile(fd, input_fd, &offset, file_info.st_size);
        if (rval == -1) {
            /* Something went wrong sending the contents of
             * /proc/issue. Write an error message. */

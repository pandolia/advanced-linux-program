...
/* Print the contents of the home page for the server's socket.
   Return a indication of success. */
void get_home_page(int socket_fd) {
	...
	/* Read from the socket. The call to read may not return
	   all the data at on time, so keep trying until we run out. */
	while (1) {
...
int main(int argc, char* const argv[]) {
	...
	/* Conert from strings to numbers. */

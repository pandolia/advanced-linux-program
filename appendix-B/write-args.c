/* Listing B.7 (write-args.c) Write the Argument List
   to a File with writev */
...
int main (int argc, char *argv[]) {
	...
	/* We'll need a "buffer" containing a newline character.
	   Use an ordinary char variable for this. */
	char newline = '\n';
	...
	/* Allocate an array of iovec elements. We'll need two for each
	   element of the argument list, one for the text itself, and
	   one for a newline. */
	...
	for (i = 0; i < argc; ++i) {
		...
		++vec_next;
		/* The second element is a single newline character. It's okay
		for multiple elements of the struct iovec array to point to the

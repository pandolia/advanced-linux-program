...
/* A handle for a temporay file created with write_temp_file.
   In this implementation, it's just a file descriptor. */
typedef int temp_file_handle;

/* Writes LENGTH bytes from BUFFER into a temporay file. The
   temporay file is immediately unlinked. Returns a handle to the
   temporary file. */
temp_file_handle write_temp_file(char* buffer, size_t length) {
	...
	/* Use the file descriptor as the handle for the
	   temporary file. */
	return fd;
}
...
char* read_temp_file(temp_file_handle temp_file, size_t* length) {
	char *buffer;
	/* The TEMP_FILE handle is a file descriptor to the
	   temporary file. */
	int fd = temp_file;
	...
	/* Close the file descriptor, which will cause the temporary
	   file to go away. */
	close(fd);
	return buffer;
}
...
int main(int argc, char *argv[]) {
	char buf[] = "test abc dfas";
	temp_file_handle temp_file = write_temp_file(buf, strlen(buf) + 1);

	int len;
	char *bufx = read_temp_file(temp_file, &len);

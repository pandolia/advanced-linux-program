 ...
 * to a line, of the process given by PID. */
void print_process_environment(pid_t pid)
{
    ...
    while (next_var < environment + length) {
        /* Print the variable. Each is NUL-terminated, so just
         * treat it like an ordinary string. */
        printf("%s\n", next_var);
        /* Advance to the next variable. Since each variable is
         * NUL-terminated, strlen counts the length of the next
         * variable, not the entire variable list. */
